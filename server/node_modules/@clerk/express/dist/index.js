...
const require_utils = require('./utils-BJYvPNh_.js');
...
let _clerk_shared_logger = require("@clerk/shared/logger");
let _clerk_shared_proxy = require("@clerk/shared/proxy");
...
const middlewareRequired = (fnName) => createErrorMessage(`The "clerkMiddleware" should be registered before using "${fnName}".
Example:
...
//#endregion
//#region src/authenticateRequest.ts
/**
* @internal
...
const authenticateRequest = (opts) => {
	...
	const clerkRequest = opts.clerkRequest ?? (0, _clerk_backend_internal.createClerkRequest)(require_utils.incomingMessageToRequest(request));
	const env = {
		...require_utils.loadApiEnv(),
		...require_utils.loadClientEnv()
	};
	...
	const isSatellite = (0, _clerk_shared_utils.handleValueOrFn)(isSatelliteInput, clerkRequest.clerkUrl, env.isSatellite);
	const domain = (0, _clerk_shared_utils.handleValueOrFn)(domainInput, clerkRequest.clerkUrl) || env.domain;
	const signInUrl = signInUrlInput || env.signInUrl;
	const proxyUrl = absoluteProxyUrl((0, _clerk_shared_utils.handleValueOrFn)(proxyUrlInput, clerkRequest.clerkUrl, env.proxyUrl), clerkRequest.clerkUrl.toString());
	if (isSatellite && !proxyUrl && !domain) throw new Error(satelliteAndMissingProxyUrlAndDomain);
	if (isSatellite && !(0, _clerk_shared_proxy.isHttpOrHttps)(signInUrl) && (0, _clerk_shared_keys.isDevelopmentFromSecretKey)(secretKey || "")) throw new Error(satelliteAndMissingSignInUrl);
...
const setResponseHeaders = (requestState, res) => {
...
*/
const setResponseForHandshake = (requestState, res) => {
	if (requestState.headers.get("location")) {
		res.status(307).end();
		return;
	}
	if (requestState.status === _clerk_backend_internal.AuthStatus.Handshake) return /* @__PURE__ */ new Error("Clerk: unexpected handshake without redirect");
};
const absoluteProxyUrl = (relativeOrAbsoluteUrl, baseUrl) => {
	if (!relativeOrAbsoluteUrl || !(0, _clerk_shared_proxy.isValidProxyUrl)(relativeOrAbsoluteUrl) || !(0, _clerk_shared_proxy.isProxyUrlRelative)(relativeOrAbsoluteUrl)) return relativeOrAbsoluteUrl;
	return new URL(relativeOrAbsoluteUrl, baseUrl).toString();
};
const resolveDefaultClerkClient = (options) => {
	if (!options.apiUrl && !options.apiVersion) return clerkClient;
	return (0, _clerk_backend.createClerkClient)({
		...require_utils.loadApiEnv(),
		...require_utils.loadClientEnv(),
...
const authenticateAndDecorateRequest = (options = {}) => {
	...
	const middleware = async (request, response, next) => {
		if (require_utils.requestHasAuthObject(request)) return next();
		if ("auth" in request) _clerk_shared_logger.logger.warnOnce("Clerk: another middleware has already set `req.auth` on this request. Clerk authentication will run anyway and overwrite it. To use another auth library alongside Clerk, configure it to store its state on a different request property.");
		let clerkRequest;
		try {
			clerkRequest = (0, _clerk_backend_internal.createClerkRequest)(require_utils.incomingMessageToRequest(request));
		} catch {
		...
		if (frontendApiProxy) {
			...
			if ((typeof frontendApiProxy.enabled === "function" ? frontendApiProxy.enabled(requestUrl) : frontendApiProxy.enabled) && (requestUrl.pathname === proxyPath || requestUrl.pathname.startsWith(proxyPath + "/"))) {
				let proxyRequest;
				try {
					proxyRequest = require_utils.requestToProxyRequest(request);
				} catch {
					response.status(400).end();
					return;
				}
				const proxyResponse = await (0, _clerk_backend_proxy.clerkFrontendApiProxy)(proxyRequest, {
					proxyPath,
					publishableKey,
					secretKey
				});
				...
				if (proxyResponse.body) {
					...
					new stream.Readable({ async read() {
						try {
		...
		try {
			...
			const auth = require_utils.brandRequestAuth((opts) => requestState.toAuth(opts));
...
const getAuth = ((req, options) => {
	if (!require_utils.requestHasAuthObject(req)) throw new Error(middlewareRequired("getAuth"));
...
const requireAuth = (options = {}) => {
	...
	return (request, response, next) => {
		(0, _clerk_shared_deprecated.deprecated)("requireAuth", "Use `clerkMiddleware()` with `getAuth()` instead. `requireAuth` will be removed in the next major version.");
		authMiddleware(request, response, (err) => {
			if (err) return next(err);
			const signInUrl = options.signInUrl || process.env.CLERK_SIGN_IN_URL || "/";
			if (!request.auth()?.userId) return response.redirect(signInUrl);
			next();
		});
	};
};
