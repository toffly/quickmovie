import { CamelToSnake, DeepPartial } from "./utils.mjs";
import { FieldId } from "./elementIds.mjs";
 ...
 *      }
 */
type UnionToRecordWithPrimitives<T extends string> = { [K in T]: string | number | boolean | Date };
 ...
 * ```
 */
type DeepLocalizationWithoutObjects<T> = { [K in keyof T]: T[K] extends LocalizationValue<any> ? T[K] : T[K] extends object ? DeepLocalizationWithoutObjects<T[K]> : T[K] };
/**
...
type __internal_LocalizationResource = {
   ...
   */
  socialButtonsBlockButtonManyInView: LocalizationValue<'provider', `${string}{{provider|titleize}}${string}`>; /** Label for the “Last used” badge on authentication strategies. */
  lastAuthenticationStrategy: LocalizationValue;
  dividerText: LocalizationValue; /** Shared strings for search inputs across the components. */
  searchInput: {
...
type UnstableErrors = WithParamName<{
  ...
  form_new_password_matches_current: LocalizationValue; /** @deprecated Use `form_password_compromised__sign_in` instead */
...
//#endregion
export { LocalizationResource, LocalizationValue, __internal_LocalizationResource };