...
var LazyOps = class {
	...
	*/
	push(op) {
		if (op.op === require_types.StepOpCode.DeferAbort) this.buffer = this.buffer.filter((buffered) => {
			return !(buffered.op === require_types.StepOpCode.DeferAdd && buffered.id === op.opts?.target_hashed_id);
		});
		this.buffer.push(op);
