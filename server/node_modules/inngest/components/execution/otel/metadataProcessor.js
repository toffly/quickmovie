...
var InngestMetadataSpanProcessor = class {
	...
	*/
	#scopeSpanKeys = /* @__PURE__ */ new Map();
	/**
	...
	*/
	#spanCleanup = new FinalizationRegistry((key) => {
		if (key) {
			...
			if (tracked) this.#scopeSpanKeys.get(tracked.scope)?.delete(key);
		}
	});
	/**
	...
	*/
	startScope({ runId, span }) {
		const { traceId, spanId } = span.spanContext();
		const scope = Symbol(`inngest.ai-metadata-scope:${runId}:${traceId}:${spanId}`);
		this.#scopeSpanKeys.set(scope, /* @__PURE__ */ new Set());
		return scope;
	}
	/**
	...
	*/
	endScope(scope) {
		...
		if (!keys) return;
		for (const key of keys) this.#spans.delete(key);
	...
	*/
	attach() {
		if (this.#attachSetup?.spanProcessorAdded) return this.#attachSetup;
		const setup = attemptProviderExtension({ processor: this });
		this.#attachSetup = setup;
		if (setup.spanProcessorAdded) processorDevDebug("attached to global OTel provider");
		return setup;
	}
	/**
	...
	*/
	declareStartingSpan({ scope, span, traceparent, onAIMetadata }) {
		if (!this.#attachSetup?.spanProcessorAdded) return;
