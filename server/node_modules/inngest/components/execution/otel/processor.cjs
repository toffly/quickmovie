		...
		if (!this.#batcher) this.#batcher = new Promise(async (resolve, reject) => {
			try {
				...
				resolve(new __opentelemetry_sdk_trace_base.BatchSpanProcessor(new __opentelemetry_exporter_trace_otlp_http.OTLPTraceExporter({
					...
					headers: {
						...app.headers,
						Authorization: `Bearer ${require_strings.hashSigningKey(app.signingKey)}`
					}
				})));
			} catch (err) {
				reject(err);
	...
	trackSpan(parentState, span, isRoot = false) {
		...
		if (!isRoot) {
			if ((span.parentSpanContext?.spanId ?? span.parentSpanId) === parentState.rootSpanId) {
				if (stepCtx) {
					const newSpanId = require_deterministicId.deterministicSpanID(stepCtx.hashedStepId);
					trackDebug("setting inngest.step.parentSpanId=%s (seed=%s) on span %s step %s index %d attempt %d", newSpanId, stepCtx.hashedStepId, spanId, stepCtx.id, stepCtx.index, stepCtx.attempt);
