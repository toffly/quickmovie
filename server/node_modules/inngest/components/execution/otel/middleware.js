...
import { behaviourToExtendedTracesBehavior, sdkFeatureObservations } from "../../sdkFeatureObservations.js";
import { clientProcessorMap } from "./access.js";
import { createProvider, extendProvider, warnDeprecatedCreateProviderBehaviour } from "./util.js";
...
const extendedTracesMiddleware = ({ behaviour = "auto", instrumentations, logLevel = DiagLogLevel.ERROR } = {}) => {
	...
	switch (behaviour) {
		case "auto": {
			...
			setProcessorReady(createProvider(behaviour, instrumentations).then((created) => {
				setup = created.setup;
				if (created.success) {
		...
		case "extendProvider": {
			...
			setup = extended.setup;
			console.warn("unable to extend provider, Extended Traces middleware will not work. Use @inngest/otel, or make sure that the provider is created and imported before the middleware is used.");
			break;
		}
		case "off": break;
