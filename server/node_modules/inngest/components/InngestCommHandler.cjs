	...
	constructor(options) {
		...
		*/
		...
		for (const fn of this.rawFns) {
			const isDefer = require_marker.isDeferredFunction(fn);
			const failureId = `${fn.id(this.client.id)}${require_InngestFunction.InngestFunction.failureSuffix}`;
	...
	async handleAsyncRequest({ timer, actions, args, getHeaders, forceExecution, fns }) {
		...
		*/
		...
		if (method === "POST") {
			...
			const fnId = await actions.queryStringWithDefaults("building requestInfo for middleware", require_consts.queryKeys.FnId);
		...
		if (shouldStream) {
			if (await actions.method("starting streaming response") === "POST") {
				const { stream, finalize } = await require_stream.createStream({ logger: this.client[require_Inngest.internalLoggerSymbol] });
				/**
	...
	*/
	async handleAction({ actions, timer, getHeaders, reqArgs, signatureValidation, body: rawBody, method, forceExecution, fns, mwInstances }) {
		...
		try {
			...
			const enableUnauthedSync = this.enableUnauthedSync ?? require_env.parseAsBoolean(this.env[require_consts.envKeys.InngestEnableUnauthedSync]) ?? true;
			if (this.client.mode === "cloud" && !enableUnauthedSync) {
				const sigCheck = await signatureValidation;
				if (!sigCheck.success) {
					this.client[require_Inngest.internalLoggerSymbol].error({
						err: sigCheck.err,
						method
			...
			if (method === "POST" || forceExecution) {
				...
				if (typeof fnId === "undefined") throw new Error("No function ID found in request");
				else if (!fn) throw new Error(`Function "${fnId}" was not found in this app; is the app synced correctly?`);
	...
	runStep({ actions, functionId, stepId, data, timer, reqArgs, headers, fn, forceExecution, headerReqVersion, requestInfo, mwInstances }) {
		...
		if (fn.fn["shouldOptimizeParallelism"]?.() === false) version$1 = require_consts.ExecutionVersion.V1;
		const result = require_promises.runAsPromise(async () => {
			...
			const requestId = await actions.headers("getting request ID for execution", require_consts.headerKeys.RequestId);
			const rawGenerationId = await actions.headers("getting generation ID for execution", require_consts.headerKeys.GenerationId);
			...
			const jobId = await actions.headers("getting job ID for execution", require_consts.headerKeys.InngestJobId);
	...
	configs(url) {
		const configs = this.rawFns.reduce((acc, fn) => [...acc, ...fn["getConfig"]({
			baseUrl: url,
			appPrefix: this.client.id
		})], []);
	...
	*/
	...
	registerBody({ url, deployId }) {
		return {
			...
			featureObservations: require_sdkFeatureObservations.sdkFeatureObservations.getJson(this.client)
		};
	}
