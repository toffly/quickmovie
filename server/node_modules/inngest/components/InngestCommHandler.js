...
import { sdkFeatureObservations } from "./sdkFeatureObservations.js";
...
var InngestCommHandler = class {
	...
	constructor(options) {
		...
		*/
		...
		for (const fn of this.rawFns) {
			...
			const failureId = `${fn.id(this.client.id)}${InngestFunction.failureSuffix}`;
	...
	async handleAsyncRequest({ timer, actions, args, getHeaders, forceExecution, fns }) {
		...
		*/
		...
		if (method === "POST") {
			...
			const fnId = await actions.queryStringWithDefaults("building requestInfo for middleware", queryKeys.FnId);
		...
		if (shouldStream) {
			if (await actions.method("starting streaming response") === "POST") {
				const { stream, finalize } = await createStream({ logger: this.client[internalLoggerSymbol] });
				/**
	...
	*/
	...
	runStep({ actions, functionId, stepId, data, timer, reqArgs, headers, fn, forceExecution, headerReqVersion, requestInfo, mwInstances }) {
		...
		if (fn.fn["shouldOptimizeParallelism"]?.() === false) version$1 = ExecutionVersion.V1;
