...
const createStepTools = (client, execution, stepHandler) => {
	...
	*/
	const createTool = (matchOp, opts) => {
		const wrappedMatchOp = (stepOptions, ...rest) => {
			...
			const alsCtx = require_als.getAsyncCtxSync()?.execution;
			if (alsCtx?.insideExperimentSelect) throw new require_NonRetriableError.NonRetriableError("Step tools (step.run, step.sleep, etc.) cannot be called inside an experiment select() callback. Move step calls into variant callbacks instead.");
	...
	*/
	const createStepMetadataWrapper = (memoizationId, builder) => {
		...
		if (!builder) builder = new require_InngestMetadata.UnscopedMetadataBuilder(client).run();
		return {
	...
	const createStepScoreWrapper = async (memoizationId, options) => {
		if (!client["experimentalScoreEnabled"]) throw new require_NonRetriableError.NonRetriableError("step.score() is experimental. Enable it by adding scoreMiddleware() from \"inngest/experimental\" to your client middleware.");
		require_InngestScore.validateStepScoreOptions(options);
		await tools.run(memoizationId, async () => {
			await require_InngestScore.sendStepScore(client, options);
		});
	};
	/**
	...
	*/
	const tools = {
		sendEvent: createTool(({ id, name }) => {
			...
			const payloadToSend = client[require_Inngest.sessionPropagationSymbol] ? require_sessions.stampPropagatedSessions(payload, ctx.sessions) : payload;
			return client["_send"]({
		...
		waitForSignal: createTool(({ id, name }, opts) => {
			return {
				...
				opts: {
					signal: opts.signal,
					timeout: require_strings.timeStr(opts.timeout, client[require_Inngest.internalLoggerSymbol]),
		...
		waitForEvent: createTool(({ id, name }, opts) => {
			const matchOpts = { timeout: require_strings.timeStr(typeof opts === "string" ? opts : opts.timeout, client[require_Inngest.internalLoggerSymbol]) };
		...
		sleep: createTool(({ id, name }, time) => {
			...
			*/
			const msTimeStr = require_strings.timeStr(time, client[require_Inngest.internalLoggerSymbol]);
			return {
				id,
				mode: require_types.StepMode.Async,
		...
		sleepUntil: createTool(({ id, name }, time) => {
			try {
				...
				*/
				client[require_Inngest.internalLoggerSymbol].warn({ err }, "Invalid `Date`, date string, `Temporal.Instant`, or `Temporal.ZonedDateTime` passed to sleepUntil");
		...
		invoke: createTool(({ id, name }, invokeOpts) => {
			...
			const opts = {
				payload: {
					data,
					v,
					meta: require_sessions.normalizeEventMeta(meta)
				},
				function_id: "",
				timeout: typeof timeout === "undefined" ? void 0 : require_strings.timeStr(timeout, client[require_Inngest.internalLoggerSymbol])
			};
			switch (_type) {
				case "fnInstance":
	...
	tools[require_InngestMetadata.metadataSymbol] = (memoizationId) => createStepMetadataWrapper(memoizationId);
	tools[require_InngestScore.scoreSymbol] = createStepScoreWrapper;
...
const invokePayloadSchema = zod_v3.z.object({
	...
	meta: zod_v3.z.object({
		sessions: zod_v3.z.record(zod_v3.z.any()).nullable().optional(),
		propagated_sessions: zod_v3.z.record(zod_v3.z.union([zod_v3.z.string(), zod_v3.z.number()])).optional()
	}).optional()
});
...
exports.experimentStepRunSymbol = experimentStepRunSymbol;
