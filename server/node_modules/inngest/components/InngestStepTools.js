...
import { getISOString, isTemporalDuration, isTemporalInstant, isTemporalZonedDateTime } from "../helpers/temporal.js";
import { timeStr } from "../helpers/strings.js";
import { getAsyncCtx, getAsyncCtxSync } from "./execution/als.js";
import { normalizeEventMeta, stampPropagatedSessions } from "../helpers/sessions.js";
	...
	*/
	const tools = {
		sendEvent: createTool(({ id, name }) => {
			...
			const payloadToSend = client[sessionPropagationSymbol] ? stampPropagatedSessions(payload, ctx.sessions) : payload;
			return client["_send"]({
		...
		waitForEvent: createTool(({ id, name }, opts) => {
			const matchOpts = { timeout: timeStr(typeof opts === "string" ? opts : opts.timeout, client[internalLoggerSymbol]) };
		...
		sleep: createTool(({ id, name }, time) => {
			...
			*/
			const msTimeStr = timeStr(time, client[internalLoggerSymbol]);
			return {
				id,
				mode: StepMode.Async,
				op: StepOpCode.Sleep,
		...
		invoke: createTool(({ id, name }, invokeOpts) => {
			const optsSchema = invokePayloadSchema.extend({ timeout: z.custom((v$1) => typeof v$1 === "number" || typeof v$1 === "string" || v$1 instanceof Date || isTemporalDuration(v$1) || isTemporalInstant(v$1) || isTemporalZonedDateTime(v$1), { message: "Invalid timeout" }).optional() });
			...
			const opts = {
				...
				function_id: "",
				timeout: typeof timeout === "undefined" ? void 0 : timeStr(timeout, client[internalLoggerSymbol])
			};
			switch (_type) {
				case "fnInstance":
...
const invokePayloadSchema = z.object({
	...
	meta: z.object({
		...
		propagated_sessions: z.record(z.union([z.string(), z.number()])).optional()
	}).optional()
});
