...
var MiddlewareManager = class {
	...
	*/
	activeWrapStepByExecution = /* @__PURE__ */ new WeakMap();
	/**
		...
		for (let i = this.middleware.length - 1; i >= 0; i--) {
			...
			if (mw?.wrapStep) {
				...
				chain = () => {
					...
					let activeWrapStep;
					if (asyncExecution) {
						const storedActiveWrapStep = this.activeWrapStepByExecution.get(asyncExecution);
						if (storedActiveWrapStep) activeWrapStep = storedActiveWrapStep;
						else activeWrapStep = /* @__PURE__ */ new Set();
					} else activeWrapStep = this.activeWrapStep;
					if (activeWrapStep.has(mw)) return next();
					let branchActiveWrapStep;
					if (asyncExecution) branchActiveWrapStep = new Set(activeWrapStep);
					else branchActiveWrapStep = activeWrapStep;
					const runWrapStep = () => {
					...
					if (!asyncContext || !asyncExecution) return runWrapStep();
					const branchExecution = { ...asyncExecution };
					this.activeWrapStepByExecution.set(branchExecution, branchActiveWrapStep);
					return require_als.runWithAsyncCtx({
						...asyncContext,
