...
const errorFactories = new Map([
	["EvalError", () => /* @__PURE__ */ new EvalError()],
	["RangeError", () => /* @__PURE__ */ new RangeError()],
	["ReferenceError", () => /* @__PURE__ */ new ReferenceError()],
	["SyntaxError", () => /* @__PURE__ */ new SyntaxError()],
	["TypeError", () => /* @__PURE__ */ new TypeError()],
	["URIError", () => /* @__PURE__ */ new URIError()],
	["NonRetriableError", () => new NonRetriableError("")]
]);
const pickErrorProperties = (subject) => {
	const result = {};
	for (const { key } of copiedErrorProperties) if (Reflect.has(subject, key)) result[key] = Reflect.get(subject, key);
	return result;
};
const reconstructError = (subject) => {
	const name = Reflect.get(subject, "name");
	const factory = typeof name === "string" ? errorFactories.get(name) : void 0;
	const output = factory ? factory() : /* @__PURE__ */ new Error();
	for (const { key, enumerable } of copiedErrorProperties) if (Reflect.has(subject, key)) Object.defineProperty(output, key, {
	...
	try {
		...
		if (typeof subject === "object" && subject !== null) {
			...
			for (let i = 0; i < maxDepth; i++) {
				if (!("cause" in target) || !target.cause) break;
				...
				const next = serializedCause === target.cause ? { ...serializedCause } : serializedCause;
				target.cause = next;
				target = next;
			}
			return ret;
		...
		try {
			return {
				...serializeError(new Error(typeof subject === "string" ? subject : stringify(subject)), false),
				stack: "",
				[SERIALIZED_KEY]: SERIALIZED_VALUE
			};
		} catch {
...
//#endregion
export { ErrCode, UnreachableError, deserializeError, errors_exports, fixEventKeyMissingSteps, getErrorMessage, rethrowError, serializeError };
