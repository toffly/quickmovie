...
const timeStr = (input, logger) => {
	...
	if (milliseconds > 0 && milliseconds < second) {
		if (logger) require_log.warnOnce(logger, "sub-second-duration-clamp", { requestedMs: milliseconds }, "Durable waits have a minimum resolution of 1s; durations under 1s round up to 1s");
		milliseconds = second;
	}
