...
const createResponse = ({ body, headers, status }) => {
	/**
	...
	const baseFn = new require_InngestCommHandler.InngestCommHandler({
		...
		handler: (reqMethod, event) => {
			return {
				...
				url: () => {
					const protocol = require_env.processEnv(require_consts.envKeys.NodeEnv) === "development" ? "http" : "https";
					return new URL(event.request.url, `${protocol}://${event.request.headers.get("host") || options.serveOrigin || ""}`);
				},
